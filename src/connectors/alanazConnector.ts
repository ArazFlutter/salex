...
import type { AlanazPayload } from '../mappers/platforms/alanazMapper';
...
const ENV_PREFIX = 'ALANAZ';
const BASE_URL = 'https://alan.az';
...
export class AlanazConnector extends BaseConnector {
  constructor() {
    super('alanaz', 'Alan.az');
  }
  ...
  async publishListing(payload: Record<string, unknown>, context?: ConnectorContext): Promise<ConnectorPublishResult> {
    const alanaz = payload as unknown as AlanazPayload;
    ...
    try {
      if (alanaz.images.length > 0) {
        console.log(`[alanaz] downloading ${alanaz.images.length} image(s)…`);
        imageDownload = await downloadImages(alanaz.images);
        if (imageDownload.skipped.length > 0) {
          console.log(`[alanaz] ${imageDownload.skipped.length} image(s) skipped`);
        }
      }
      ...
      await this.selectCategoryPath(driver, alanaz.categoryPath, timeoutMs);
      await this.fillListingForm(driver, alanaz, imageDownload, timeoutMs);
      const result = await this.submitAndExtractResult(driver, timeoutMs);

      result.publishMetadata = {
        ...result.publishMetadata,
        listingTitle: alanaz.title,
        listingPrice: alanaz.price,
        listingCity: alanaz.city,
      };
      ...
      throw new Error(`Alan.az publish failed: ${message}`);
    } finally {
      if (imageDownload) {
  ...
  override getListingUrl(result: ConnectorPublishResult): string | null {
    ...
    // Never return /search?q= — it is not a listing detail page and was incorrectly marked as success.
    return null;
  }
  ...
  override async fetchListingUrl(result: ConnectorPublishResult, context?: ConnectorContext): Promise<string | null> {
    ...
    try {
      ...
      if (candidateUrl) {
        ...
        if (url) {
          console.log('[alanaz] recovery: resolved via direct URL');
      ...
      if (listingTitle && userId) {
        ...
        if (url) {
          console.log('[alanaz] recovery: resolved via user listings search');
          await persistSessionCookies(driver, userId, this.platformId);
          return url;
        }
      }
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`Alan.az Selenium recovery failed: ${message}`);
    } finally {
      await driver.quit().catch(() => undefined);
    }
  }
  ...
  override normalizeError(error: unknown) {
    if (error instanceof Error) {
      if (error.message.startsWith('Alan.az publish failed:')) {
        return { code: 'ALANAZ_PUBLISH_ERROR', message: error.message };
      }

      if (error.message.includes('login failed') || error.message.includes('Session expired and login failed')) {
        return { code: 'ALANAZ_LOGIN_ERROR', message: error.message };
      }

      if (error.message.startsWith('Alan.az Selenium recovery failed:')) {
        return { code: 'ALANAZ_SELENIUM_RECOVERY_ERROR', message: error.message };
      }
    }
    ...
    try {
      return await driver.executeScript<boolean>(`
        return !!(
          ...
          document.querySelector('a[href*="/cabinet"], a[href*="/profile"], a[href*="/hesabim"]') ||
          document.querySelector('[class*="logged-in"], [class*="user-info"], [class*="UserMenu"]') ||
          document.querySelector('a[href*="/elanlarim"], a[href*="/my-ads"]')
        );
      `);
    } catch {
      return false;
  ...
  private async performLogin(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    const phone = process.env.ALANAZ_LOGIN_PHONE?.trim();

    if (!phone) {
      console.log('[alanaz] ALANAZ_LOGIN_PHONE not configured — skipping login');
      return false;
    }
    ...
    try {
      ...
      if (!loginOpened) {
        console.log('[alanaz] login form could not be opened');
        return false;
      }

      const phoneEntered = await this.enterPhoneNumber(driver, phone, timeoutMs);
      if (!phoneEntered) {
        console.log('[alanaz] could not enter phone number');
        return false;
      }

      const otpRequested = await this.submitPhoneForOtp(driver, timeoutMs);
      if (!otpRequested) {
        console.log('[alanaz] could not submit phone number for OTP');
        return false;
      }

      const otp = await this.acquireOtpCode(phone);
      if (!otp) {
        console.log('[alanaz] OTP acquisition timed out or failed');
        return false;
      }

      const otpEntered = await this.enterOtpCode(driver, otp);
      if (!otpEntered) {
        console.log('[alanaz] could not enter OTP code');
        return false;
      }
      ...
      if (!success) {
        console.log('[alanaz] login did not complete — session not valid after OTP');
        return false;
      }

      console.log('[alanaz] login successful');
      return true;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      console.log(`[alanaz] login failed: ${msg}`);
      return false;
    }
  }
  ...
  private async openLoginForm(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    const clicked = await driver.executeScript<boolean>(`
      ...
      var byHref = document.querySelector('a[href*="login"], a[href*="signin"], a[href*="auth"], a[href*="giris"]');
  ...
  private async submitPhoneForOtp(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    ...
    try {
      ...
      console.log('[alanaz] OTP input field not detected — continuing');
    }
  ...
  private async acquireOtpCode(phone: string): Promise<string | null> {
    const staticOtp = process.env.ALANAZ_OTP_CODE?.trim();
    if (staticOtp && /^\d{4,6}$/.test(staticOtp)) {
      console.log('[alanaz] using OTP from ALANAZ_OTP_CODE env var');
      return staticOtp;
    }
  ...
  private async pollOtpFromFile(phone: string): Promise<string | null> {
    const otpFilePath = process.env.ALANAZ_OTP_FILE?.trim()
      || path.join(process.cwd(), '.alanaz-otp');
    const otpTimeoutMs = Number(process.env.ALANAZ_OTP_TIMEOUT_MS) || DEFAULT_OTP_TIMEOUT_MS;

    try {
      fs.writeFileSync(otpFilePath, 'WAITING', 'utf-8');
    } catch (err) {
      console.log(`[alanaz] could not write OTP file at ${otpFilePath}: ${err}`);
      return null;
    }

    console.log('─'.repeat(60));
    console.log(`[alanaz] OTP required for phone: ${phone}`);
    console.log(`[alanaz] Write the OTP code to: ${otpFilePath}`);
    console.log(`[alanaz] Timeout: ${Math.round(otpTimeoutMs / 1000)}s`);
    console.log(`[alanaz] Example: echo 1234 > "${otpFilePath}"`);
    console.log('─'.repeat(60));
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (match) {
          console.log('[alanaz] OTP code received from file');
    ...
    console.log('[alanaz] OTP timeout — no code received');
    try { fs.unlinkSync(otpFilePath); } catch { /* ignore */ }
    return null;
  }
  ...
  private async navigateToNewListingPage(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    for (const url of NEW_LISTING_CANDIDATES) {
      try {
        ...
        `);
        if (hasForm) {
          console.log(`[alanaz] new listing page found at: ${url}`);
          return true;
        }
      } catch { continue; }
    }
    ...
    const clicked = await driver.executeScript<boolean>(`
      ...
      var keywords = ['elan ver', 'elan yerləşdir', 'yeni elan', 'add'];
      for (var i = 0; i < links.length; i++) {
        var text = (links[i].textContent || '').trim().toLowerCase();
        if (keywords.some(function(k) { return text.includes(k); })) {
          links[i].click();
    ...
    console.log('[alanaz] could not find new listing page');
    return false;
  }
  ...
  private async selectCategoryPath(driver: WebDriver, path: string[], timeoutMs: number): Promise<void> {
    if (path.length === 0) {
      throw new Error('Alan.az categoryPath is empty — mapping must supply full navigation path');
    }
    await selectCategoryPathByText(driver, path, timeoutMs, 'alanaz');
  }
    ...
    try {
      await driver.wait(async () => {
        return await driver.executeScript<boolean>(`
          return !!(
            document.querySelector('form') ||
            document.querySelector('input[name="title"], input[name="name"], input[name="ad_title"], input[name="bashliq"]') ||
      ...
      console.log('[alanaz] form detection timed out — attempting to fill anyway');
    }
  }
  ...
  private async fillField(driver: WebDriver, value: string, selectors: string[]): Promise<void> {
    ...
    if (!element) {
      console.log(`[alanaz] could not find field — tried: ${selectors[0]}…`);
      return;
    }
    await element.clear();
  ...
  private async fillTextField(driver: WebDriver, value: string, selectors: string[]): Promise<void> {
    const element = await this.findFirstVisible(driver, selectors);
    if (!element) {
      console.log(`[alanaz] could not find text field — tried: ${selectors[0]}…`);
      return;
    }
  ...
  private async selectLocation(driver: WebDriver, location: string): Promise<void> {
    const selected = await driver.executeScript<boolean>(`
      ...
      var buttons = document.querySelectorAll('[class*="city"] button, [class*="location"] button, [class*="region"] a, [class*="sheher"] a');
    ...
    if (!selected) {
      console.log(`[alanaz] location "${location}" not matched — form may use default`);
    }
  }
  ...
  private async uploadImages(driver: WebDriver, download: DownloadResult): Promise<void> {
    ...
    if (localPaths.length === 0) {
      console.log('[alanaz] no local images to upload');
      return;
    }
    ...
    try {
      ...
      if (fileInputs.length === 0) {
        console.log('[alanaz] no file input found on page — skipping image upload');
        return;
      }
      ...
      if (acceptsMultiple !== null) {
        ...
        console.log(`[alanaz] uploaded ${localPaths.length} image(s) via multi-file input`);
      } else {
        await fileInput.sendKeys(localPaths[0]);
        console.log('[alanaz] uploaded 1 image via single-file input');
            ...
            for (const input of freshInputs) {
              try {
                ...
                if (!val) {
                  ...
                  uploaded = true;
                  console.log(`[alanaz] uploaded image ${i + 1}/${localPaths.length}`);
                  break;
                }
              } catch { continue; }
            }
            if (!uploaded) {
              console.log(`[alanaz] no available file input for image ${i + 1} — stopping`);
              break;
            }
          } catch {
            console.log(`[alanaz] failed to upload image ${i + 1} — stopping`);
            break;
          }
        }
      }
      ...
      const msg = error instanceof Error ? error.message : String(error);
      console.log(`[alanaz] image upload error: ${msg} — continuing without images`);
    }
  }
  ...
  private async collectPostSubmitSignals(driver: WebDriver, urlBefore: string): Promise<PostSubmitSignals> {
    ...
    const urlIdMatch = urlAfter.match(ALANAZ_ID_RE);
    ...
    const dom = await driver.executeScript<{
      ...
      var metaIdMatch = metaUrl.match(/_i(\\d+)/);
  ...
  private async trySubmit(driver: WebDriver): Promise<boolean> {
    ...
    return await driver.executeScript<boolean>(`
      var buttons = Array.from(document.querySelectorAll('button, input[type="submit"], a.btn'));
      var keywords = ['yerləşdir', 'dərc et', 'göndər', 'submit', 'опубликовать', 'publish', 'elan yerləşdir', 'paylaş'];
      for (var i = 0; i < buttons.length; i++) {
        var text = (buttons[i].textContent || buttons[i].value || '').trim().toLowerCase();
        if (keywords.some(function(k) { return text.includes(k); })) {
          buttons[i].click();
  ...
  private async searchUserListingsForMatch(
    ...
    const listingUrl = await driver.executeScript<string | null>(`
      ...
      var cards = document.querySelectorAll(
        '.ad-card, [class*="listing-item"], [class*="ad-item"], [class*="elan-item"], a[href*="_i"]'
      );
      ...
      for (var i = 0; i < cards.length; i++) {
        ...
        if (score > bestScore) {
          var link = card.tagName === 'A' ? card : card.querySelector('a[href*="_i"]');
    ...
    if (!listingUrl) {
      console.log('[alanaz] no matching listing found on user listings page');
      return null;
    }

    console.log(`[alanaz] matched listing on user page: ${listingUrl}`);
  ...
  private async navigateToUserListings(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    ...
    const foundLink = await driver.executeScript<string | null>(`
      ...
      var keywords = ['elanlarım', 'elanlarim', 'my ads', 'мои объявления', 'hesabım'];
      for (var i = 0; i < links.length; i++) {
        var text = (links[i].textContent || '').trim().toLowerCase();
        ...
        if (/\\/elanlarim|\\/my-?ads|\\/cabinet|\\/hesabim|\\/profile/.test(href)) return links[i].href;
      }
      return null;
    `);
    ...
    if (foundLink) {
      ...
      const hasContent = await driver.executeScript<boolean>(`
        return !!(document.querySelector('.ad-card, [class*="listing-item"], [class*="elan-item"], a[href*="_i"]'));
      `);
    ...
    console.log('[alanaz] could not navigate to user listings page');
    return false;
  }
  ...
  private async extractPageUrl(driver: WebDriver, candidateUrl: string): Promise<string | null> {
    ...
    if (pageData.currentUrl !== candidateUrl && pageData.currentUrl.includes('alan.az')) {
