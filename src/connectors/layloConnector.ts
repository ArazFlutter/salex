...
import type { LayloPayload } from '../mappers/platforms/layloMapper';
import { selectCategoryPathByText } from './categoryPathNav';
...
const LAYLO_ID_RE = /\/ilan-detay\/(\d+)/;
...
export class LayloConnector extends BaseConnector {
  ...
  async publishListing(payload: Record<string, unknown>, context?: ConnectorContext): Promise<ConnectorPublishResult> {
    const laylo = payload as unknown as LayloPayload;
    ...
    try {
      if (laylo.images.length > 0) {
        console.log(`[laylo] downloading ${laylo.images.length} image(s)…`);
        imageDownload = await downloadImages(laylo.images);
        if (imageDownload.skipped.length > 0) {
          console.log(`[laylo] ${imageDownload.skipped.length} image(s) skipped`);
        }
      }
      ...
      await this.selectCategoryPath(driver, laylo.categoryPath, timeoutMs);
      await this.fillListingForm(driver, laylo, imageDownload, timeoutMs);
      const result = await this.submitAndExtractResult(driver, timeoutMs);

      result.publishMetadata = {
        ...result.publishMetadata,
        listingTitle: laylo.title,
        listingPrice: laylo.price,
        listingCity: laylo.city,
      };
      ...
      throw new Error(`Laylo.az publish failed: ${message}`);
    } finally {
      if (imageDownload) {
  ...
  override getListingUrl(result: ConnectorPublishResult): string | null {
    ...
    return `${BASE_URL}/ilan-detay/${result.externalListingId}`;
  }
  ...
  override async fetchListingUrl(result: ConnectorPublishResult, context?: ConnectorContext): Promise<string | null> {
    ...
    if (!candidateUrl && !listingTitle) return null;
    ...
    try {
      ...
      if (candidateUrl) {
        ...
        if (url) {
          console.log('[laylo] recovery: resolved via direct URL');
      ...
      if (listingTitle && userId) {
        ...
        if (url) {
          console.log('[laylo] recovery: resolved via user listings search');
          await persistSessionCookies(driver, userId, this.platformId);
          return url;
        }
      }
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`Laylo.az Selenium recovery failed: ${message}`);
    } finally {
      await driver.quit().catch(() => undefined);
    }
  }
  ...
  override normalizeError(error: unknown) {
    if (error instanceof Error) {
      if (error.message.startsWith('Laylo.az publish failed:')) {
        return { code: 'LAYLO_PUBLISH_ERROR', message: error.message };
      }

      if (error.message.includes('login failed') || error.message.includes('Session expired and login failed')) {
        return { code: 'LAYLO_LOGIN_ERROR', message: error.message };
      }

      if (error.message.startsWith('Laylo.az Selenium recovery failed:')) {
        return { code: 'LAYLO_SELENIUM_RECOVERY_ERROR', message: error.message };
      }
    }
    ...
    try {
      return await driver.executeScript<boolean>(`
        return !!(
          ...
          document.querySelector('a[href*="/cabinet"], a[href*="/profile"], a[href*="/hesabim"], a[href*="/panel"]') ||
          document.querySelector('[class*="logged-in"], [class*="user-info"], [class*="UserMenu"]') ||
          document.querySelector('a[href*="/elanlarim"], a[href*="/my-ads"], a[href*="cixis"]')
        );
      `);
    } catch {
      return false;
  ...
  private async performLogin(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    const phone = process.env.LAYLO_LOGIN_PHONE?.trim();

    if (!phone) {
      console.log('[laylo] LAYLO_LOGIN_PHONE not configured — skipping login');
      return false;
    }
    ...
    try {
      ...
      if (!loginOpened) {
        console.log('[laylo] login form could not be opened');
        return false;
      }

      const phoneEntered = await this.enterPhoneNumber(driver, phone, timeoutMs);
      if (!phoneEntered) {
        console.log('[laylo] could not enter phone number');
        return false;
      }

      const otpRequested = await this.submitPhoneForOtp(driver, timeoutMs);
      if (!otpRequested) {
        console.log('[laylo] could not submit phone number for OTP');
        return false;
      }

      const otp = await this.acquireOtpCode(phone);
      if (!otp) {
        console.log('[laylo] OTP acquisition timed out or failed');
        return false;
      }

      const otpEntered = await this.enterOtpCode(driver, otp);
      if (!otpEntered) {
        console.log('[laylo] could not enter OTP code');
        return false;
      }
      ...
      if (!success) {
        console.log('[laylo] login did not complete — session not valid after OTP');
        return false;
      }

      console.log('[laylo] login successful');
      return true;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      console.log(`[laylo] login failed: ${msg}`);
      return false;
    }
  }
  ...
  private async openLoginForm(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    const clicked = await driver.executeScript<boolean>(`
      ...
      var keywords = ['daxil ol', 'giriş', 'войти', 'login', 'sign in', 'qeydiyyat'];
      ...
      var byHref = document.querySelector('a[href*="login"], a[href*="signin"], a[href*="auth"], a[href*="giris"], a[href*="daxil"]');
    ...
    try {
      await driver.wait(async () => {
        return await driver.executeScript<boolean>(`
          return !!(
            document.querySelector('input[type="tel"], input[name="phone"], input[name="login"], input[name="email"]') ||
  ...
  private async submitPhoneForOtp(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    const submitted = await driver.executeScript<boolean>(`
      ...
      var keywords = ['göndər', 'davam', 'daxil', 'giriş', 'next', 'send', 'отправить', 'continue', 'təsdiq'];
    ...
    try {
      ...
      console.log('[laylo] OTP input field not detected — continuing');
    }
  ...
  private async acquireOtpCode(phone: string): Promise<string | null> {
    const staticOtp = process.env.LAYLO_OTP_CODE?.trim();
    if (staticOtp && /^\d{4,6}$/.test(staticOtp)) {
      console.log('[laylo] using OTP from LAYLO_OTP_CODE env var');
      return staticOtp;
    }
  ...
  private async pollOtpFromFile(phone: string): Promise<string | null> {
    const otpFilePath = process.env.LAYLO_OTP_FILE?.trim()
      || path.join(process.cwd(), '.laylo-otp');
    const otpTimeoutMs = Number(process.env.LAYLO_OTP_TIMEOUT_MS) || DEFAULT_OTP_TIMEOUT_MS;

    try {
      fs.writeFileSync(otpFilePath, 'WAITING', 'utf-8');
    } catch (err) {
      console.log(`[laylo] could not write OTP file at ${otpFilePath}: ${err}`);
      return null;
    }

    console.log('─'.repeat(60));
    console.log(`[laylo] OTP required for phone: ${phone}`);
    console.log(`[laylo] Write the OTP code to: ${otpFilePath}`);
    console.log(`[laylo] Timeout: ${Math.round(otpTimeoutMs / 1000)}s`);
    console.log(`[laylo] Example: echo 1234 > "${otpFilePath}"`);
    console.log('─'.repeat(60));
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (match) {
          console.log('[laylo] OTP code received from file');
    ...
    console.log('[laylo] OTP timeout — no code received');
    try { fs.unlinkSync(otpFilePath); } catch { /* ignore */ }
    return null;
  }
  ...
  private async navigateToNewListingPage(driver: WebDriver, timeoutMs: number): Promise<boolean> {
      ...
      try {
        ...
        const hasForm = await driver.executeScript<boolean>(`
          return !!(
            ...
            (document.querySelector('input[name="title"], input[name="name"], textarea[name="description"], textarea') ||
             document.querySelector('input[type="file"]'))
          );
        `);
        if (hasForm) {
          console.log(`[laylo] new listing page found at: ${url}`);
          return true;
        }
      } catch { continue; }
    }
    ...
    const clicked = await driver.executeScript<boolean>(`
      ...
      var keywords = ['elan yerləşdir', 'elan ver', 'yeni elan', 'ilan ekle', 'add'];
      for (var i = 0; i < links.length; i++) {
        var text = (links[i].textContent || '').trim().toLowerCase();
        if (keywords.some(function(k) { return text.includes(k); })) {
          links[i].click();
    ...
    console.log('[laylo] could not find new listing page');
    return false;
  }
  ...
  private async selectCategoryPath(driver: WebDriver, path: string[], timeoutMs: number): Promise<void> {
    if (path.length === 0) {
      throw new Error('Laylo.az categoryPath is empty — mapping must supply full navigation path');
    }
    await selectCategoryPathByText(driver, path, timeoutMs, 'laylo');
  }
  ...
  private async fillListingForm(
    ...
    await this.fillField(driver, payload.title, [
      ...
      'input[placeholder*="elan adı" i]',
      'input[placeholder*="ad" i]',
      'input[placeholder*="название" i]',
      'input[data-testid="title"]',
    ]);

    await this.fillTextField(driver, payload.description, [
      ...
      'textarea[placeholder*="mətn" i]',
      'textarea[placeholder*="açıqlama" i]',
      'textarea[placeholder*="описание" i]',
      '[contenteditable="true"]',
      'textarea[data-testid="description"]',
    ]);

    await this.fillField(driver, String(payload.price), [
      'input[name="price"]',
      'input[name="qiymet"]',
      'input[name="amount"]',
      'input[name="fiyat"]',
    ...
    await this.selectLocation(driver, payload.city);
    ...
    try {
      await driver.wait(async () => {
        return await driver.executeScript<boolean>(`
          return !!(
            document.querySelector('form') ||
            document.querySelector('input[name="title"], input[name="name"], input[name="bashliq"], input[name="elan_adi"]') ||
            document.querySelector('textarea[name="description"], textarea[name="text"], textarea[name="metn"]') ||
            document.querySelector('input[name="price"], input[name="qiymet"]')
          );
        `);
      }, timeoutMs);
    } catch {
      console.log('[laylo] form detection timed out — attempting to fill anyway');
    }
  }
  ...
  private async fillField(driver: WebDriver, value: string, selectors: string[]): Promise<void> {
    ...
    if (!element) {
      console.log(`[laylo] could not find field — tried: ${selectors[0]}…`);
      return;
    }
    await element.clear();
  ...
  private async fillTextField(driver: WebDriver, value: string, selectors: string[]): Promise<void> {
    const element = await this.findFirstVisible(driver, selectors);
    if (!element) {
      console.log(`[laylo] could not find text field — tried: ${selectors[0]}…`);
      return;
    }
  ...
  private async selectLocation(driver: WebDriver, location: string): Promise<void> {
    const selected = await driver.executeScript<boolean>(`
      var selects = document.querySelectorAll('select[name="city"], select[name="region"], select[name="location"], select[name="sheher"], select[name="seher"]');
      ...
      var buttons = document.querySelectorAll('[class*="city"] button, [class*="location"] button, [class*="sheher"] a, [class*="seher"] a');
    ...
    if (!selected) {
      console.log(`[laylo] location "${location}" not matched — form may use default`);
    }
  }
  ...
  private async uploadImages(driver: WebDriver, download: DownloadResult): Promise<void> {
    ...
    if (localPaths.length === 0) {
      console.log('[laylo] no local images to upload');
      return;
    }
    ...
    try {
      ...
      if (fileInputs.length === 0) {
        console.log('[laylo] no file input found on page — skipping image upload');
        return;
      }
      ...
      if (acceptsMultiple !== null) {
        ...
        console.log(`[laylo] uploaded ${localPaths.length} image(s) via multi-file input`);
      } else {
        await fileInput.sendKeys(localPaths[0]);
        console.log('[laylo] uploaded 1 image via single-file input');
            ...
            for (const input of freshInputs) {
              try {
                ...
                if (!val) {
                  ...
                  uploaded = true;
                  console.log(`[laylo] uploaded image ${i + 1}/${localPaths.length}`);
                  break;
                }
              } catch { continue; }
            }
            if (!uploaded) {
              console.log(`[laylo] no available file input for image ${i + 1} — stopping`);
              break;
            }
          } catch {
            console.log(`[laylo] failed to upload image ${i + 1} — stopping`);
            break;
          }
        }
      }
      ...
      const msg = error instanceof Error ? error.message : String(error);
      console.log(`[laylo] image upload error: ${msg} — continuing without images`);
    }
  }
      ...
      await driver.wait(async () => {
        return await driver.executeScript<boolean>(`
          ...
          var formFields = document.querySelectorAll(
            'form input[name="title"], form input[name="name"], form textarea[name="description"]'
          );
  ...
  private async collectPostSubmitSignals(driver: WebDriver, urlBefore: string): Promise<PostSubmitSignals> {
    ...
    const urlIdMatch = urlAfter.match(LAYLO_ID_RE);
    ...
    const dom = await driver.executeScript<{
      ...
      var metaIdMatch = metaUrl.match(/\\/ilan-detay\\/(\\d+)/);
      ...
      if (/elan.*qəbul/i.test(bodyLower)) successIndicators.push('az_listing_accepted');
      ...
      var formStillVisible = !!(
        document.querySelector('form input[name="title"], form input[name="name"], form input[name="bashliq"]') &&
        document.querySelector('form textarea[name="description"], form textarea[name="text"], form textarea[name="metn"]')
      );
  ...
  private classifyPublishOutcome(s: PostSubmitSignals): ConnectorPublishResult {
    ...
    throw new Error('No confirmation of successful publish — page state inconclusive');
  }
  ...
  private async trySubmit(driver: WebDriver): Promise<boolean> {
    ...
    return await driver.executeScript<boolean>(`
      var buttons = Array.from(document.querySelectorAll('button, input[type="submit"], a.btn'));
      var keywords = ['yerləşdir', 'dərc et', 'göndər', 'submit', 'опубликовать', 'publish', 'elan yerləşdir', 'paylaş', 'əlavə et'];
      for (var i = 0; i < buttons.length; i++) {
        var text = (buttons[i].textContent || buttons[i].value || '').trim().toLowerCase();
        if (keywords.some(function(k) { return text.includes(k); })) {
          buttons[i].click();
  ...
  private async searchUserListingsForMatch(
    ...
    const listingUrl = await driver.executeScript<string | null>(`
      ...
      var cards = document.querySelectorAll(
        '.ad-card, [class*="listing-item"], [class*="ad-item"], [class*="elan-item"], [class*="elan_item"], a[href*="/ilan-detay/"]'
      );
      ...
      for (var i = 0; i < cards.length; i++) {
        ...
        if (targetPrice !== null) {
          var priceEl = card.querySelector('[class*="price"], [class*="qiymet"]');
          if (priceEl) {
        ...
        if (score > bestScore) {
          var link = card.tagName === 'A' ? card : card.querySelector('a[href*="/ilan-detay/"]');
    ...
    if (!listingUrl) {
      console.log('[laylo] no matching listing found on user listings page');
      return null;
    }

    console.log(`[laylo] matched listing on user page: ${listingUrl}`);
  ...
  private async navigateToUserListings(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    ...
    const foundLink = await driver.executeScript<string | null>(`
      ...
      var keywords = ['elanlarım', 'elanlarim', 'ilanlarım', 'ilanlarim', 'my ads', 'мои объявления', 'hesabım', 'panel'];
      for (var i = 0; i < links.length; i++) {
        var text = (links[i].textContent || '').trim().toLowerCase();
        ...
        if (/\\/elanlarim|\\/ilanlarim|\\/my-?ads|\\/cabinet|\\/hesabim|\\/panel|\\/profilim/.test(href)) return links[i].href;
      }
      return null;
    `);
    ...
    if (foundLink) {
      ...
      const hasContent = await driver.executeScript<boolean>(`
        return !!(document.querySelector('.ad-card, [class*="listing-item"], [class*="elan-item"], [class*="elan_item"], [class*="ilan-item"], a[href*="/ilan-detay/"]'));
      `);
    ...
    console.log('[laylo] could not navigate to user listings page');
    return false;
  }
  ...
  private async extractPageUrl(driver: WebDriver, candidateUrl: string): Promise<string | null> {
    ...
    if (pageData.currentUrl !== candidateUrl && pageData.currentUrl.includes('laylo.az')) {
      return pageData.currentUrl;
    }

    if (/404|not found|tapılmadı/i.test(pageData.title) && pageData.currentUrl === candidateUrl) {
      return null;
    }
  ...
  async startLoginWithPhone(
    ...
    try {
      ...
      console.error('[laylo] startLoginWithPhone error:', err);
  ...
  async completeLoginWithOtp(
    ...
    try {
      ...
      console.error('[laylo] completeLoginWithOtp error:', err);
      return false;
    }
  }
