...
import type { BirjacomPayload } from '../mappers/platforms/birjacomMapper';
...
const ENV_PREFIX = 'BIRJACOM';
const BASE_URL = 'https://birja.com';
...
export class BirjacomConnector extends BaseConnector {
  constructor() {
    super('birjacom', 'Birja.com');
  }
  ...
  async publishListing(payload: Record<string, unknown>, context?: ConnectorContext): Promise<ConnectorPublishResult> {
    const birja = payload as unknown as BirjacomPayload;
    ...
    try {
      if (birja.images.length > 0) {
        console.log(`[birjacom] downloading ${birja.images.length} image(s)…`);
        imageDownload = await downloadImages(birja.images);
        if (imageDownload.skipped.length > 0) {
          console.log(`[birjacom] ${imageDownload.skipped.length} image(s) skipped`);
        }
      }
      ...
      await this.selectCategoryPath(driver, birja.categoryPath, timeoutMs);
      await this.fillListingForm(driver, birja, imageDownload, timeoutMs);
      const result = await this.submitAndExtractResult(driver, timeoutMs);

      result.publishMetadata = {
        ...result.publishMetadata,
        listingTitle: birja.title,
        listingPrice: birja.price,
        listingCity: birja.city,
      };
      ...
      throw new Error(`Birja.com publish failed: ${message}`);
    } finally {
      if (imageDownload) {
  ...
  override getListingUrl(result: ConnectorPublishResult): string | null {
    ...
    return `${BASE_URL}/ads/az/${result.externalListingId}`;
  }
  ...
  override async fetchListingUrl(result: ConnectorPublishResult, context?: ConnectorContext): Promise<string | null> {
    ...
    try {
      ...
      if (candidateUrl) {
        ...
        if (url) {
          console.log('[birjacom] recovery: resolved via direct URL');
      ...
      if (listingTitle && userId) {
        ...
        if (url) {
          console.log('[birjacom] recovery: resolved via user listings search');
          await persistSessionCookies(driver, userId, this.platformId);
          return url;
        }
      }
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`Birja.com Selenium recovery failed: ${message}`);
    } finally {
      await driver.quit().catch(() => undefined);
    }
  }
  ...
  override normalizeError(error: unknown) {
    if (error instanceof Error) {
      if (error.message.startsWith('Birja.com publish failed:')) {
        return { code: 'BIRJACOM_PUBLISH_ERROR', message: error.message };
      }

      if (error.message.includes('login failed') || error.message.includes('Session expired and login failed')) {
        return { code: 'BIRJACOM_LOGIN_ERROR', message: error.message };
      }

      if (error.message.startsWith('Birja.com Selenium recovery failed:')) {
        return { code: 'BIRJACOM_SELENIUM_RECOVERY_ERROR', message: error.message };
      }
    }
    ...
    try {
      return await driver.executeScript<boolean>(`
        return !!(
          ...
          document.querySelector('a[href*="/elanlarim"], a[href*="/my-ads"], a[href*="logout"], a[href*="cixis"]')
        );
      `);
    } catch {
      return false;
  ...
  private async performLogin(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    const phone = process.env.BIRJACOM_LOGIN_PHONE?.trim();

    if (!phone) {
      console.log('[birjacom] BIRJACOM_LOGIN_PHONE not configured — skipping login');
      return false;
    }
    ...
    try {
      ...
      if (!loginOpened) {
        console.log('[birjacom] login form could not be opened');
        return false;
      }

      const phoneEntered = await this.enterPhoneNumber(driver, phone, timeoutMs);
      if (!phoneEntered) {
        console.log('[birjacom] could not enter phone number');
        return false;
      }

      const otpRequested = await this.submitPhoneForOtp(driver, timeoutMs);
      if (!otpRequested) {
        console.log('[birjacom] could not submit phone number for OTP');
        return false;
      }

      const otp = await this.acquireOtpCode(phone);
      if (!otp) {
        console.log('[birjacom] OTP acquisition timed out or failed');
        return false;
      }

      const otpEntered = await this.enterOtpCode(driver, otp);
      if (!otpEntered) {
        console.log('[birjacom] could not enter OTP code');
        return false;
      }
      ...
      if (!success) {
        console.log('[birjacom] login did not complete — session not valid after OTP');
        return false;
      }

      console.log('[birjacom] login successful');
      return true;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      console.log(`[birjacom] login failed: ${msg}`);
      return false;
    }
  }
  ...
  private async openLoginForm(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    ...
    } catch { /* form may already be visible */ }
  ...
  private async submitPhoneForOtp(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    ...
    try {
      ...
      console.log('[birjacom] OTP input field not detected — continuing');
    }
  ...
  private async acquireOtpCode(phone: string): Promise<string | null> {
    const staticOtp = process.env.BIRJACOM_OTP_CODE?.trim();
    if (staticOtp && /^\d{4,6}$/.test(staticOtp)) {
      console.log('[birjacom] using OTP from BIRJACOM_OTP_CODE env var');
      return staticOtp;
    }
  ...
  private async pollOtpFromFile(phone: string): Promise<string | null> {
    const otpFilePath = process.env.BIRJACOM_OTP_FILE?.trim()
      || path.join(process.cwd(), '.birjacom-otp');
    const otpTimeoutMs = Number(process.env.BIRJACOM_OTP_TIMEOUT_MS) || DEFAULT_OTP_TIMEOUT_MS;

    try {
      fs.writeFileSync(otpFilePath, 'WAITING', 'utf-8');
    } catch (err) {
      console.log(`[birjacom] could not write OTP file at ${otpFilePath}: ${err}`);
      return null;
    }

    console.log('─'.repeat(60));
    console.log(`[birjacom] OTP required for phone: ${phone}`);
    console.log(`[birjacom] Write the OTP code to: ${otpFilePath}`);
    console.log(`[birjacom] Timeout: ${Math.round(otpTimeoutMs / 1000)}s`);
    console.log(`[birjacom] Example: echo 1234 > "${otpFilePath}"`);
    console.log('─'.repeat(60));
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (match) {
          console.log('[birjacom] OTP code received from file');
    ...
    console.log('[birjacom] OTP timeout — no code received');
    try { fs.unlinkSync(otpFilePath); } catch { /* ignore */ }
    return null;
  }

  // =========================================================================
  //  Private — navigate to new listing page (multi-URL fallback)
  // =========================================================================
  ...
  private async navigateToNewListingPage(driver: WebDriver, timeoutMs: number): Promise<boolean> {
      ...
      try {
        ...
        const hasForm = await driver.executeScript<boolean>(`
          return !!(
            ...
            (document.querySelector('input[name="title"], input[name="name"], textarea[name="description"]') ||
             document.querySelector('input[type="file"]'))
          );
        `);
        if (hasForm) {
          console.log(`[birjacom] new listing page found at: ${url}`);
          return true;
        }
      } catch { continue; }
    }
    ...
    const clicked = await driver.executeScript<boolean>(`
      ...
      var keywords = ['elan ver', 'elan yerləşdir', 'yeni elan', 'add', 'post'];
      for (var i = 0; i < links.length; i++) {
        var text = (links[i].textContent || '').trim().toLowerCase();
        if (keywords.some(function(k) { return text.includes(k); })) {
          links[i].click();
    ...
    console.log('[birjacom] could not find new listing page');
    return false;
  }
  ...
  private async selectCategoryPath(driver: WebDriver, path: string[], timeoutMs: number): Promise<void> {
    if (path.length === 0) {
      throw new Error('Birja.com categoryPath is empty — mapping must supply full navigation path');
    }
    await selectCategoryPathByText(driver, path, timeoutMs, 'birjacom');
  }
    ...
    try {
      await driver.wait(async () => {
        return await driver.executeScript<boolean>(`
          return !!(
            document.querySelector('form') ||
            document.querySelector('input[name="title"], input[name="name"], input[name="ad_title"]') ||
            document.querySelector('textarea[name="description"], textarea[name="text"]') ||
            document.querySelector('input[name="price"], input[name="qiymet"]')
          );
        `);
      }, timeoutMs);
    } catch {
      console.log('[birjacom] form detection timed out — attempting to fill anyway');
    }
  }
  ...
  private async fillField(driver: WebDriver, value: string, selectors: string[]): Promise<void> {
    ...
    if (!element) {
      console.log(`[birjacom] could not find field — tried: ${selectors[0]}…`);
      return;
    }
    await element.clear();
  ...
  private async fillTextField(driver: WebDriver, value: string, selectors: string[]): Promise<void> {
    const element = await this.findFirstVisible(driver, selectors);
    if (!element) {
      console.log(`[birjacom] could not find text field — tried: ${selectors[0]}…`);
      return;
    }
  ...
  private async selectLocation(driver: WebDriver, location: string): Promise<void> {
    const selected = await driver.executeScript<boolean>(`
      var selects = document.querySelectorAll('select[name="city"], select[name="region"], select[name="location"], select[name="sheher"]');
    ...
    if (!selected) {
      console.log(`[birjacom] location "${location}" not matched — form may use default`);
    }
  }
  ...
  private async uploadImages(driver: WebDriver, download: DownloadResult): Promise<void> {
    ...
    if (localPaths.length === 0) {
      console.log('[birjacom] no local images to upload');
      return;
    }
    ...
    try {
      ...
      if (fileInputs.length === 0) {
        console.log('[birjacom] no file input found on page — skipping image upload');
        return;
      }
      ...
      if (acceptsMultiple !== null) {
        ...
        console.log(`[birjacom] uploaded ${localPaths.length} image(s) via multi-file input`);
      } else {
        await fileInput.sendKeys(localPaths[0]);
        console.log('[birjacom] uploaded 1 image via single-file input');
            ...
            for (const input of freshInputs) {
              try {
                ...
                if (!val) {
                  ...
                  uploaded = true;
                  console.log(`[birjacom] uploaded image ${i + 1}/${localPaths.length}`);
                  break;
                }
              } catch { continue; }
            }
            if (!uploaded) {
              console.log(`[birjacom] no available file input for image ${i + 1} — stopping`);
              break;
            }
          } catch {
            console.log(`[birjacom] failed to upload image ${i + 1} — stopping`);
            break;
          }
        }
      }
      ...
      const msg = error instanceof Error ? error.message : String(error);
      console.log(`[birjacom] image upload error: ${msg} — continuing without images`);
    }
  }
  ...
  private async collectPostSubmitSignals(driver: WebDriver, urlBefore: string): Promise<PostSubmitSignals> {
    ...
    const urlIdMatch = urlAfter.match(BIRJA_ID_RE);
    ...
    const dom = await driver.executeScript<{
      ...
      var metaIdMatch = metaUrl.match(/\\/ads\\/az\\/(\\d+)/);
      ...
      var formStillVisible = !!(
        document.querySelector('form input[name="title"], form input[name="name"]') &&
        document.querySelector('form textarea[name="description"], form textarea[name="text"]')
      );
  ...
  private async trySubmit(driver: WebDriver): Promise<boolean> {
    ...
    return await driver.executeScript<boolean>(`
      var buttons = Array.from(document.querySelectorAll('button, input[type="submit"], a.btn'));
      var keywords = ['yerləşdir', 'dərc et', 'göndər', 'submit', 'опубликовать', 'publish', 'elan ver', 'paylaş', 'əlavə et'];
      for (var i = 0; i < buttons.length; i++) {
        var text = (buttons[i].textContent || buttons[i].value || '').trim().toLowerCase();
        if (keywords.some(function(k) { return text.includes(k); })) {
          buttons[i].click();
  ...
  private async searchUserListingsForMatch(
    ...
    const listingUrl = await driver.executeScript<string | null>(`
      ...
      var cards = document.querySelectorAll(
        '.ad-card, [class*="listing-item"], [class*="ad-item"], [class*="elan-item"], a[href*="/ads/az/"]'
      );
      ...
      for (var i = 0; i < cards.length; i++) {
        ...
        if (score > bestScore) {
          var link = card.tagName === 'A' ? card : card.querySelector('a[href*="/ads/az/"]');
    ...
    if (!listingUrl) {
      console.log('[birjacom] no matching listing found on user listings page');
      return null;
    }

    console.log(`[birjacom] matched listing on user page: ${listingUrl}`);
  ...
  private async navigateToUserListings(driver: WebDriver, timeoutMs: number): Promise<boolean> {
    ...
    const foundLink = await driver.executeScript<string | null>(`
      ...
      var keywords = ['elanlarım', 'elanlarim', 'my ads', 'мои объявления', 'hesabım', 'panel', 'cabinet'];
      for (var i = 0; i < links.length; i++) {
        var text = (links[i].textContent || '').trim().toLowerCase();
        ...
        if (/\\/elanlarim|\\/my-?ads|\\/cabinet|\\/hesabim|\\/panel|\\/profile|\\/user\/az\//.test(href)) return links[i].href;
      }
      return null;
    `);
    ...
    if (foundLink) {
      ...
      const hasContent = await driver.executeScript<boolean>(`
        return !!(document.querySelector('.ad-card, [class*="listing-item"], [class*="elan-item"], a[href*="/ads/az/"]'));
      `);
    ...
    console.log('[birjacom] could not navigate to user listings page');
    return false;
  }
  ...
  private async extractPageUrl(driver: WebDriver, candidateUrl: string): Promise<string | null> {
    ...
    if (pageData.currentUrl !== candidateUrl && pageData.currentUrl.includes('birja.com')) {
