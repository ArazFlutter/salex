...
import { clearGlobalAuthSession, sendOtp, verifyOtp, directRegister } from '../services/otpService';
...
export async function directRegisterController(request: Request, response: Response) {
  const phone = String(request.body?.phone ?? '');
  const result = await directRegister(phone);
